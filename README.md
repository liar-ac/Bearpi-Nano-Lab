...
```mermaid
flowchart LR
  Board["BearPi-HMNano<br/>E53_IA1传感器"] -->|"HTTP/JSON<br/>X-Device-Token"| API["DjangoRESTAPI"]
  API -->|"写入"| DB[("MySQL")]
  API -->|"可选ChannelLayer"| Redis[("Redis")]
  API -->|"WebSocket<br/>/ws/realtime"| Web["Vue3Web控制台"]
  API -->|"RESTAPI"| Mobile["uni-app移动端"]
  Web -->|"批量控制/规则配置"| API
  Mobile -->|"查看/控制/告警确认"| API
  API -->|"设备命令队列"| Board
  API --> Cloud["CloudDeviceStatus<br/>预留华为云IoTDA集成"]
```
