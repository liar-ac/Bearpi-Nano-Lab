...
|能力|已经实现的内容|
|---|---|
|设备接入|开发板按SN自动注册，A001-A004固定槽位，其余设备自动分配1-120号槽位|
...
flowchart TB
  subgraph Client["客户端层"]
    Web["Vue3Web控制台<br/>总览/拓扑/功耗/任务/规则"]
    Screen["实时大屏<br/>WebSocket订阅"]
    Mobile["uni-app移动端<br/>H5/微信小程序"]
  end

  subgraph Access["接入与认证层"]
    Static["静态资源入口<br/>Django托管dist或Nginx"]
    UserAuth["用户认证<br/>JWT+RBAC"]
    DeviceAuth["设备认证<br/>HMAC-SHA256Token"]
  end

  subgraph Service["应用服务层:DjangoASGI"]
    REST["RESTAPI<br/>DRFView/Serializer"]
    Realtime["实时推送<br/>DjangoChannels"]
    DeviceGateway["设备网关<br/>自动注册/槽位映射"]
    Telemetry["遥测服务<br/>Ingest/History/Aggregation"]
    Command["命令服务<br/>Queue/Batch/Retry/Ack"]
    AlarmAudit["规则告警与审计<br/>Rule/Alarm/AuditLog"]
  end

  subgraph Data["数据与集成层"]
    MySQL[("MySQL<br/>Device/Sensor/RawPoint/Alarm")]
    Redis[("Redis可选<br/>ChannelLayer")]
    Cloud["CloudDeviceStatus<br/>预留华为云IoTDA"]
  end

  subgraph Device["设备层"]
    Board["BearPi-HMNano<br/>OpenHarmony/Hi3861"]
    Sensor["E53_IA1<br/>温湿度/光照/电机"]
    Firmware["HTTP固件<br/>Telemetry/Pull/Ack"]
  end
  ...
  Sensor --> Firmware
  Firmware --> DeviceAuth
  DeviceAuth --> DeviceGateway
  DeviceGateway --> Telemetry
  Firmware <-->|"命令拉取/执行回执"| Command
